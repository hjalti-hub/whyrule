...
#: handler returns immediately for files that are not memory files. SessionStart
#: runs once and may walk a large project, so it gets more room.
EVENT_TIMEOUTS = {"PostToolUse": 20, "SessionStart": 30}


def _importable_from_anywhere() -> bool:
    """True when ``python -m whyrule`` works from any directory.

    Running out of a clone puts that directory on ``sys.path``, so
    ``python -m whyrule`` succeeds there and nowhere else. A hook runs from the
    ...
    try:
        with tempfile.TemporaryDirectory() as neutral:
            result = subprocess.run(
                [sys.executable, "-c", "import whyrule"],
                cwd=neutral,
                capture_output=True,
                timeout=30,
            )
...
def hook_command() -> str | None:
    ...
    """
    if shutil.which("whyrule"):
        return "whyrule hook"
    if _importable_from_anywhere():
        return f"{sys.executable} -m whyrule hook"
    return None


#: Shown when whyrule is only reachable from its own checkout.
NOT_INSTALLED = (
    "whyrule: hooks not installed.\n"
    "\n"
    "whyrule runs fine from this directory, but a hook runs from whatever "
    "project you have open, and it would not be able to import whyrule from "
...
def hook_config(command: str | None = None) -> dict:
    """The hooks block whyrule wants to own."""
...
def _write(path: Path, settings: dict) -> str | None:
    ...
    try:
        ...
        if path.exists():
            shutil.copy2(path, path.with_suffix(path.suffix + ".whyrule-backup"))
...
def install(path: Path, *, command: str | None = None, dry_run: bool = False) -> tuple[int, str]:
    ...
    if write_error:
        return 2, f"whyrule: {write_error}"

    verb = "already installed in" if already else "installed into"
    return 0, (
        f"whyrule hooks {verb} {path}\n"
        "  SessionStart  reports instructions that are already being dropped\n"
        "  PostToolUse   re-checks the memory files the moment one is edited\n"
...
def uninstall(path: Path) -> tuple[int, str]:
    ...
    if error:
        return 2, f"whyrule: {error}"
    if not settings:
        return 0, f"whyrule: nothing to remove from {path}"

    removed = _strip(settings)
    if not removed:
        return 0, f"whyrule: no whyrule hooks found in {path}"

    write_error = _write(path, settings)
    if write_error:
        return 2, f"whyrule: {write_error}"
    return 0, f"whyrule: removed {removed} hook entr(ies) from {path}"
...
def status(path: Path) -> tuple[int, str]:
    ...
    if not installed:
        return 1, (
            f"whyrule hooks are not installed in {path}\n"
            "Run `whyrule install` to have Claude check its own instructions "
            "without being asked."
        )
    return 0, f"whyrule hooks installed in {path}: {', '.join(installed)}"
